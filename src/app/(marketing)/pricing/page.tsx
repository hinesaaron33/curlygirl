...
export default function PricingPage() {
  ...
  return (
    <>
      {/* Header */}
      <section className="relative overflow-hidden bg-gradient-to-b from-base-light via-base to-base-dark py-24 sm:py-32">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <div className="absolute -top-20 right-[10%] h-[300px] w-[300px] rounded-full bg-white/20 blur-[80px]" />
          <div className="absolute bottom-0 left-[15%] h-[200px] w-[200px] rounded-full bg-pink/10 blur-[80px]" />
        </div>

        <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-[13px] font-semibold tracking-[0.2em] text-pink uppercase">Pricing</p>
            <h1 className="mt-4 font-[family-name:var(--font-playfair)] text-4xl font-bold tracking-tight text-ink sm:text-5xl">
              Choose your plan
            </h1>
          ...
          <div className="mx-auto mt-16 grid max-w-5xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {tiers.map((tier) => (
              <div
                key={tier.name}
                className={`relative flex h-full flex-col rounded-2xl border bg-white/30 p-8 shadow-sm backdrop-blur-sm transition-all duration-400 hover:-translate-y-1.5 hover:shadow-lg ${
                  tier.highlighted
                    ? "border-pink/30 ring-4 ring-pink/10"
                    : "border-white/40"
                }`}
              >
                {tier.highlighted && (
                  <div className="absolute -top-3.5 left-6">
                    <span className="rounded-full bg-gradient-to-r from-pink to-blush px-4 py-1 text-[10px] font-bold tracking-wider text-white uppercase shadow-lg shadow-pink/25">
                      Most Popular
                    </span>
                  </div>
                )}
                ...
                <div className="mt-6 flex items-baseline gap-1">
                  ...
                  {tier.period && <span className="text-sm text-ink/40">{tier.period}</span>}
                </div>
                {yearly && tier.yearlyPrice !== "$0" && (
                  <p className="mt-1 text-xs text-ink/40">billed annually</p>
                )}
                ...
                <div className="mt-8">
                  <Link
                    ...
                    className={`block w-full rounded-xl py-3.5 text-center text-sm font-semibold transition-all ${
                      tier.highlighted
                        ? "bg-pink text-white shadow-lg shadow-pink/25 hover:bg-pink-dark hover:shadow-xl hover:-translate-y-0.5"
                        : "border-2 border-ink/15 bg-white/40 text-ink/70 hover:border-ink/25 hover:bg-white/60 hover:text-ink"
                    }`}
                  >
                    GET STARTED
                  </Link>
